	...
	<div class="rightObj">
		<ul class="joinForm">			
			<li class="email list">
				...
				<span class="input"><input type="text" name="email" id="email" placeholder="¿Ã∏ﬁ¿œ¿ª ¿‘∑¬«œººø‰."/></span><div style="clear:both"></div>
				<button type="button" class="btnClose"></button>
			</li>			
		</ul>
		<button type="submit" class="btnRegist">¿Ã∏ﬁ¿œ ¿Œ¡ı π◊ ∫Ø∞Ê«œ±‚</button>
	</div>
</form>
<script>
var date    = new Date();
...
$("#joinForm").submit(function(){
	...
	if(trim($("#email").val())==""){
		MAlert("¿Ã∏ﬁ¿œ¿ª ¿‘∑¬«œººø‰.", $("#email").parent());
		$("#Nname").focus();
		return false;
	};
	if(Echk($("#email").val())==false){
		alert("ø√πŸ∏• ¿Ã∏ﬁ¿œ «¸Ωƒ¿Ã æ∆¥’¥œ¥Ÿ.");
		MAlert("ø√πŸ∏• ¿Ã∏ﬁ¿œ «¸Ωƒ¿Ã æ∆¥’¥œ¥Ÿ.", $("#email").parent());
		$("#Nname").focus();
		return false;
	};
