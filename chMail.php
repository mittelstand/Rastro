	...
	<div class="rightObj">
		<ul class="joinForm">			
			<li class="email list">
				...
				<span class="input"><input type="text" name="email" id="email" placeholder="ÀÌ¸ÞÀÏÀ» ÀÔ·ÂÇÏ¼¼¿ä."/></span><div style="clear:both"></div>
				<button type="button" class="btnClose"></button>
			</li>			
		</ul>
		<button type="submit" class="btnRegist">¶ó½ºÆ®·Î ½ÃÀÛÇÏ±â</button>
	</div>
</form>
<script>
var date    = new Date();
...
$("#joinForm").submit(function(){
	...
	if(trim($("#Nname").val())==""){
		MAlert("½Ç¸íÀ» ÀÔ·ÂÇÏ¼¼¿ä.", $("#Nname").parent());
		$("#Nname").focus();
		return false;
	};
	if(Nchk($("#Nname").val())==false){
		alert("¿Ã¹Ù¸¥ ½Ç¸í Çü½ÄÀÌ ¾Æ´Õ´Ï´Ù.");
		MAlert("¿Ã¹Ù¸¥ ½Ç¸í Çü½ÄÀÌ ¾Æ´Õ´Ï´Ù.", $("#Nname").parent());
		$("#Nname").focus();
		return false;	
	};
	if(trim($("#email").val())==""){
		MAlert("ÀÌ¸ÞÀÏÀ» ÀÔ·ÂÇÏ¼¼¿ä.", $("#email").parent());
		$("#Nname").focus();
		return false;
	};
	if(Echk($("#email").val())==false){
		alert("¿Ã¹Ù¸¥ ÀÌ¸ÞÀÏ Çü½ÄÀÌ ¾Æ´Õ´Ï´Ù.");
		MAlert("¿Ã¹Ù¸¥ ÀÌ¸ÞÀÏ Çü½ÄÀÌ ¾Æ´Õ´Ï´Ù.", $("#email").parent());
		$("#Nname").focus();
		return false;
	};
	if(trim($("#pwd").val())==""){
		MAlert("ºñ¹Ð¹øÈ£¸¦ ÀÔ·ÂÇÏ¼¼¿ä.", $("#pwd").parent());
		$("#Nname").focus();
		return false;
	}
	if(Pchk($("#pwd").val())==false){
		alert("¿Ã¹Ù¸¥ ºñ¹Ð¹øÈ£ Çü½ÄÀÌ ¾Æ´Õ´Ï´Ù.");
		MAlert("¿Ã¹Ù¸¥ ºñ¹Ð¹øÈ£ Çü½ÄÀÌ ¾Æ´Õ´Ï´Ù.", $("#pwd").parent());
		$("#Nname").focus();
		return false;	
	}
	sw = 0
